...
export const SlotInstanceEditor: React.FC<ModuleComponentProps<SlotInstanceStoredProps>> = ({
  ...
  return (
    <div {...nodeWrapperProps} className={styles.container} data-dukafi-slot-instance="">
      <div className={styles.header} data-dukafi-slot-instance-header="">
        <TargetSolidIcon size={11} color="currentColor" aria-hidden="true" />
        <span className={styles.label} data-dukafi-slot-label="">Slot: {slotName}</span>
      </div>
      <div className={styles.content} data-dukafi-slot-instance-content="">{children}</div>
    </div>
  )
}
