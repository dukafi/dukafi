...
export const FormModule: ModuleDefinition<FormProps> = {
  ...
  render: (props, renderedChildren) => {
    ...
    const attrs = [
      `data-dukafi-form-id="${formId}"`,
      `data-dukafi-form-mode="${props.mode}"`,
      props.mode === 'cms' ? `data-dukafi-target-table="${props.targetTableId}"` : '',
      props.mode === 'custom' ? `action="${safeUrl(props.action)}"` : '',
      props.mode === 'custom' ? `method="${props.method}"` : '',
      props.successBehavior === 'message' ? `data-dukafi-success-message="${props.successMessage}"` : '',
      props.successBehavior === 'redirect' ? `data-dukafi-success-redirect="${safeUrl(props.redirectUrl)}"` : '',
    ].filter(Boolean).join(' ')
    ...
    const honeypot = props.mode === 'cms'
      ? `<input type="text" name="${props.honeypotName}" autocomplete="off" tabindex="-1" data-dukafi-honeypot hidden>`
      : ''
    return {
...
export const LabelModule: ModuleDefinition<LabelProps> = {
  ...
  render: (props) => {
    ...
    return { html: `<label data-dukafi-label-target="auto">${props.text}</label>` }
  },
}
...
export const InputModule: ModuleDefinition<InputProps> = {
  ...
  render: (props) => ({ html: `<input${attrs([
    ['data-dukafi-form-control', 'input'],
    ['data-dukafi-field-id', props.fieldId],
...
export const TextareaModule: ModuleDefinition<TextareaProps> = {
  ...
  render: (props) => ({ html: `<textarea${attrs([
    ['data-dukafi-form-control', 'textarea'],
    ['data-dukafi-field-id', props.fieldId],
    ['name', props.name || props.fieldId],
    ['id', props.id],
    ['placeholder', props.placeholder],
...
export const SelectModule: ModuleDefinition<SelectProps> = {
  ...
  render: (props, renderedChildren) => ({
    html: `<select${attrs([
      ['data-dukafi-form-control', 'select'],
      ['data-dukafi-field-id', props.fieldId],
      ['name', props.name || props.fieldId],
      ['id', props.id],
...
export const FormMessageModule: ModuleDefinition<FormMessageProps> = {
  ...
  htmlTag: 'div',
  render: (props) => ({
    html: `<div data-dukafi-form-message="${props.kind}" data-dukafi-form-id="${normalizeIdentifierValue(props.formId)}" role="${props.kind === 'error' ? 'alert' : 'status'}">${props.text}</div>`,
  }),
}
  ...
  return {
    ...
    render: (props) => ({
      html: `<input type="${args.inputType}"${attrs([
        ['data-dukafi-form-control', args.inputType],
        ['data-dukafi-field-id', props.fieldId],
        ['name', props.name || props.fieldId],
        ['id', props.id],
        ['value', props.value],
