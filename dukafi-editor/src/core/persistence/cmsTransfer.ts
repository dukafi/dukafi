...
export function submitSiteBundleExport(opts: ExportRequest): void {
  ...
  const targetName = `dukafi-export-${Date.now()}-${Math.random().toString(36).slice(2)}`
...
export function parseSiteBundleArchive(bytes: Uint8Array): SiteBundle | null {
  ...
  const manifestBytes =
    entries[BUNDLE_ARCHIVE_MANIFEST_PATH] ?? entries[LEGACY_BUNDLE_ARCHIVE_MANIFEST_PATH]
