 ...
 *    `<dukafi-hole>`. The top-level `publishPage` owns these, initialises
...
export interface RenderConfig {
   ...
   * When a node's id is in this set, `renderNode` emits a `<dukafi-hole>`
  ...
  /**
   * Monotonic publish version stamped into every `<dukafi-hole data-dukafi-version>`
...
export interface RenderAccumulators {
  ...
  /**
   * Each node that actually emits a `<dukafi-hole>` during the render walk
   * calls `acc.holeNodeIds.add(nodeId)`. After the walk completes, `render.ts`
   * reads `.size > 0` to decide whether to inject the
   * `/_dukafi/hole-runtime.js` script tag into the `<head>`. This is more
