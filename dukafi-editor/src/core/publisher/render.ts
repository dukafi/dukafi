...
interface PublishPageOptions {
   ...
   * "Load more" endpoint URLs in `data-dukafi-loop-endpoint` attributes
   ...
   *   pre-built site CSS bundle (`/_dukafi/css/<filename>`). The HTML stays small,
   ...
   * value, e.g. `'/_dukafi/css/'`. Defaults to `'/_dukafi/css/'`.
   */
   ...
   * `/_dukafi/runtime/cache/<hash>/...` paths served from the host.
   */
  runtimePackageImportmap?: PublishedRuntimePackageImportmap
  /**
   * Monotonic publish version from `server/publish/publishState.ts`.
   * Stamped into every `<dukafi-hole data-dukafi-version>` attribute so the hole
...
function buildStyleHead(
  ...
  if (cssEmission === 'external') {
    ...
    const baseUrl = options.cssAssetBaseUrl ?? '/_dukafi/css/'
...
interface RuntimeAssetsBlock {
  ...
  /** `<script type="module" src="/_dukafi/hole-runtime.js" defer>` or empty string. */
...
function buildRuntimeAssetsBlock(
  ...
  const loopEndpointBaseUrl = options.loopEndpointBaseUrl ?? '/_dukafi/loop/'
  const loopRuntimeScript = hasInfiniteLoops
    ? `  <script type="module" src="/_dukafi/assets/loop-runtime.js" data-dukafi-loop-endpoint="${escapeHtml(loopEndpointBaseUrl)}" defer></script>`
    : ''
  ...
  const holeRuntimeScript = hasHoles
    ? `  <script type="module" src="/_dukafi/hole-runtime.js?v=${options.publishVersion ?? 0}" defer></script>`
    : ''
...
interface AssembledDocumentParts {
  ...
  importmapTag: string
  headRuntimeScripts: string
  /** `<script type="module" src="/_dukafi/hole-runtime.js" defer>` or empty. */
  holeRuntimeScript: string
  bodyOpenTag: string
  bodyHtml: string
  bodyEndRuntimeScripts: string
...
export function publishPage(
  ...
  // emit <dukafi-hole> placeholders instead of recursing.
  ...
  // `<dukafi-hole>` to `holeNodeIds`. After the walk, the head builders
  // read their `.size`.
