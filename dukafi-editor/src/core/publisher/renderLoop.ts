 ...
 *   - 'infinite': items emitted, plus a `data-dukafi-loop-id` sentinel and the
 ...
 *     `/_dukafi/loop/<loopId>?page=N` and appends rendered HTML.
 *
...
export function renderLoop(
  ...
  if (!data) {
    return `<!-- dukafi: loop "${escapeHtml(loopId)}" has no resolved data -->`
  }

  const variants = node.children ?? []
  if (variants.length === 0) {
    return '<!-- dukafi: loop has no child template -->'
  }
  ...
  let attrs = ` data-dukafi-loop="${escapeHtml(loopId)}"`
  attrs += ` data-dukafi-loop-page="${data.pageNumber}"`
  if (isInfinite) {
    attrs += ` data-dukafi-loop-mode="infinite"`
    attrs += ` data-dukafi-loop-has-more="${data.hasMore ? 'true' : 'false'}"`
    attrs += ` data-dukafi-loop-page-size="${typeof props.pageSize === 'number' ? Math.floor(props.pageSize) : 10}"`
