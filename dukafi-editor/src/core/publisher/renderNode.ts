...
function renderStandardNode(
  ...
  // module JS is served as an external file (`/_dukafi/module-js/<id>.js`),
...
/**
 * Emit a `<dukafi-hole>` placeholder for a dynamic node.
 *
...
function renderHolePlaceholder(
  ...
  return (
    `<dukafi-hole id="hole-${safeId}" data-dukafi-hole="${safeId}" data-dukafi-version="${version}" style="display:contents">` +
    sanitized +
    `</dukafi-hole>`
  )
}
...
export function renderNode(
  ...
  if (!def) {
    ...
    return `<!-- dukafi: unknown module "${escapeHtml(node.moduleId)}" -->`
  }

  // Layer C: when this node id is in the dynamic set, emit a <dukafi-hole>
  // placeholder and do NOT recurse into the subtree. The hole runtime will
  // fetch the full rendered fragment at request time via /_dukafi/hole/<nodeId>.
