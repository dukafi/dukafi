 ...
 * Dukafi install). Deliberately NOT a generic interchange format: no HTML/CSS
 ...
 * Dukafi's Ruby backend never implemented).
 *
...
export interface DukafiPageExportFile {
  dukafyExport: 'page'
  version: number
  exportedAt: number
  page: {
...
export function buildPageExport(page: Page, siteClasses: Record<string, StyleRule>): DukafiPageExportFile {
  return {
    dukafyExport: 'page',
...
export function parsePageExport(raw: unknown): DukafiPageExportFile | null {
 ...
 *    imported verbatim.
 */
export function importPageIntoSite(site: SiteDocument, exported: DukafiPageExportFile): Page {
...
export interface DukafiSiteExportFile {
  dukafyExport: 'site'
  version: number
  exportedAt: number
  site: SiteDocument
...
export function buildSiteExport(site: SiteDocument): DukafiSiteExportFile {
 ...
 * is a persisted feature in Dukafi today (both are Instatic-era `data_rows`
 * concepts this fork never wired to a Ruby backend), so there is nothing
 * real to round-trip.
 */
export function parseSiteExport(raw: unknown): DukafiSiteExportFile | null {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null
  const r = raw as Record<string, unknown>
