 ...
 *
 *   import { definePluginCanvasOverlay } from '@dukafi/plugin-sdk'
 *   import { useCanvasNodeRect, useEditorStore } from '@dukafi/host-hooks'
 *
 *   function PluginPin() {
 ...
 *     stays contained
 *
 * The plugin's bundle externalizes `react` / `@dukafi/host-ui` /
 * `@dukafi/host-hooks` / `@dukafi/plugin-sdk` like every other
 ...
 * surface during `dukafi-plugin build`, not at editor activation.
 */
