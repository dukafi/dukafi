...
export interface RenderOutput {
   ...
   * (`/_dukafi/module-js/<moduleId>.js`) on published pages — never inlined,
...
export interface NodeWrapperProps {
   ...
   */
  'data-dukafi-readonly-label'?: string
  'data-dukafi-readonly-kind'?: 'page' | 'component'
  'data-dukafi-readonly-id'?: string
  /**
...
> {
   ...
   * dynamic. Layer C uses it to emit `<dukafi-hole>` placeholders around the
  ...
  /**
   * Optional loading state rendered at publish time into the `<dukafi-hole>`
   ...
   *
   * If omitted, the `<dukafi-hole>` element is empty (zero visible content
