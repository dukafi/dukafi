  ...
  extensions: [
    {
      ...
      tokenizer(src: string) {
        ...
        return { type: 'dukafiVideo', raw: match[0], href: match[1].trim() }
      },
      renderer() {
...
function tokenToBlockNode(token: Token): JSONNode | JSONNode[] | null {
  switch (token.type) {
    ...
    default:
      // Custom tokens (e.g. dukafiVideo)
      if (token.type === 'dukafiVideo') {
...
/**
 * Expand inline `<u>` … `</u>` HTML pairs into synthetic `dukafiUnderline`
 ...
 * marked child group (em / strong / del / link / dukafiUnderline), so nested
...
function pushInlineGroup(out: JSONNode[], tokens: Token[], marks: JSONMark[]): void {
  const expanded = expandInlineHtmlMarkPairs(tokens, '<u>', '</u>', 'dukafiUnderline')
