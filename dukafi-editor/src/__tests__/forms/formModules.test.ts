...
describe('base form primitive modules', () => {
  ...
  it('renders a CMS-native form with runtime metadata and children', () => {
    ...
    expect(output.html).toContain('data-dukafi-form-id="newsletter"')
    expect(output.html).toContain('data-dukafi-form-mode="cms"')
    expect(output.html).toContain('data-dukafi-success-message="Thanks"')
  ...
  it('renders labels and text-like controls as semantic HTML', () => {
    expect(LabelModule.render({ text: 'Email', targetMode: 'auto', targetId: '' }, []).html)
      .toBe('<label data-dukafi-label-target="auto">Email</label>')
    ...
    expect(input).toContain('data-dukafi-form-control="input"')
    expect(input).toContain('data-dukafi-field-id="email"')
  ...
  it('renders textarea, select, option, and option-group primitives', () => {
    expect(TextareaModule.render({
      ...
      minLength: 0,
      maxLength: 500,
    }, []).html).toBe('<textarea data-dukafi-form-control="textarea" data-dukafi-field-id="message" name="message" id="message-input" placeholder="Message" rows="4" maxlength="500">Hello</textarea>')
    ...
    expect(SelectModule.render({
      ...
      .toBe('<select data-dukafi-form-control="select" data-dukafi-field-id="plan" name="plan" id="plan-select" required><option value="pro">Pro</option></select>')
  })
  ...
  it('renders choice controls, submit, and form messages', () => {
    expect(CheckboxModule.render({
      ...
      required: true,
      disabled: false,
    }, []).html).toBe('<input type="checkbox" data-dukafi-form-control="checkbox" data-dukafi-field-id="agree" name="agree" id="agree-input" value="yes" checked required>')
    ...
    expect(RadioModule.render({
      ...
      required: false,
      disabled: false,
    }, []).html).toBe('<input type="radio" data-dukafi-form-control="radio" data-dukafi-field-id="plan" name="plan" id="plan-pro" value="pro">')
    ...
    expect(FormMessageModule.render({
      ...
      kind: 'success',
      text: 'Thanks',
    }, []).html).toBe('<div data-dukafi-form-message="success" data-dukafi-form-id="newsletter" role="status">Thanks</div>')
  })
  ...
  it('normalizes configured form ids while preserving safe custom-action URLs', () => {
    ...
    expect(formOutput.html).toContain('data-dukafi-form-id="Contact-Form"')
    expect(formOutput.html).toContain('action="https://example.com/submit"')
    expect(formOutput.html).toContain('data-dukafi-success-redirect="#"')
    ...
    expect(FormMessageModule.render({ ...FormMessageModule.defaults, formId: 'Contact Form!' }, []).html)
      .toContain('data-dukafi-form-id="Contact-Form"')
  })
})
