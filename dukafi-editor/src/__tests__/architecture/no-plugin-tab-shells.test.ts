 ...
 * @dukafi/host-ui (src/ui/components/Tabs/) for any tab UI. Rolling a
 ...
 * test. Fix: use <Tabs> / <TabList> from @dukafi/host-ui instead of a
 * hand-rolled tablist.
 *
  ...
  it('no file outside the allowlist contains role="tablist" directly', () => {
    ...
    throw new Error(
      `[no-plugin-tab-shells] ${violations.length} file(s) contain role="tablist" outside the allowlist.\n` +
        `Use <Tabs> / <TabList> from @dukafi/host-ui instead of a hand-rolled tablist.\n\n` +
        `Violations:\n` +
        lines.join('\n') +
