...
describe('ModuleSandboxFrame srcDoc', () => {
  it('builds an isolated iframe document with import map, encoded module source, and host bridge', () => {
    const threeUrl = '/_dukafi/runtime/cache/abc123/three/build/three.module.js'
    const srcDoc = createSandboxSrcDoc({
      ...
      importMap: {
        imports: {
          three: threeUrl,
          'three/': '/_dukafi/runtime/cache/abc123/three/',
        },
      },
      context: {
    ...
    expect(srcDoc).toContain('dukafi-module-sandbox')
    expect(srcDoc).toContain('dukafi-module-host')
