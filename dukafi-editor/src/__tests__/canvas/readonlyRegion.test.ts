 ...
 * straight to the nearest `[data-dukafi-readonly-*]` ancestor wrongly treated
...
function readonly(tag: string): HTMLElement {
  ...
  el.setAttribute('data-dukafi-readonly-label', 'Main template')
  el.setAttribute('data-dukafi-readonly-kind', 'page')
  el.setAttribute('data-dukafi-readonly-id', 'tpl-main')
  return el
}
...
describe('closestReadonlyRegion', () => {
  ...
  it('returns the region element for read-only template chrome', () => {
    ...
    expect(closestReadonlyRegion(nav)?.getAttribute('data-dukafi-readonly-label')).toBe('Main template')
  })
  ...
  it('returns the read-only ancestor for an unmarked child of chrome (e.g. a logo image)', () => {
    ...
    expect(region?.getAttribute('data-dukafi-readonly-id')).toBe('tpl-main')
    expect(region?.getAttribute('data-dukafi-readonly-kind')).toBe('page')
  })
