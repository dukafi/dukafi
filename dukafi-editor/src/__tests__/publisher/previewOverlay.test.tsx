...
describe('publishPage — 2-node tree golden test (Phase 7)', () => {
  ...
  const headingModule = makeModule('base.text', {
    ...
    render: (props) => ({
      html: `<h1 class="dukafi-heading">${props['text'] ?? ''}</h1>`,
      css: '/* base.text */\n.dukafi-heading { font-family: sans-serif; margin: 0; }',
    }),
  })
  ...
  it('renders a 2-node tree (root + heading) to a complete HTML document', () => {
    ...
    // Page content
    expect(html).toContain('<h1 class="dukafi-heading">Hello World</h1>')

    // CSS injection (deduplicated)
    expect(html).toContain('.dukafi-heading { font-family: sans-serif; margin: 0; }')
