...
const loopModule = makeModule('base.loop', {
  ...
  render: () => ({ html: '<!-- dukafi: loop default render hit -->' }),
})
...
describe('publisher loop renderer', () => {
  ...
  it('attaches data attrs and registers infinite mode', () => {
    ...
    expect(html).toContain('data-dukafi-loop="loop"')
    expect(html).toContain('data-dukafi-loop-mode="infinite"')
    expect(html).toContain('data-dukafi-loop-has-more="true"')
    expect(html).toContain('data-dukafi-loop-page-size="2"')
    // Loop runtime script injected when at least one infinite loop exists
    expect(html).toContain('/_dukafi/assets/loop-runtime.js')
  })
