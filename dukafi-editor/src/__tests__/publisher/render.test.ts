...
describe('renderNode', () => {
  ...
  const containerDef: ModuleDefinition<{ className: string }> = makeModule(
    'base.container',
    {
      ...
      render: (props, children) => ({
        ...
        css: '.dukafi-container { display: block; }',
      }),
    },
  )
  ...
  it('emits HTML comment for unknown moduleId', () => {
    ...
    expect(html).toContain('<!-- dukafi: unknown module')
  ...
  describe('class injection — parent classes never bleed into descendants', () => {
    ...
    it('multi-class on root: prepends to existing class on the same element', () => {
      ...
      const classedDef = makeModule('base.classed', {
        render: () => ({ html: '<button class="dukafi-btn">Click</button>' }),
      })
      ...
      expect(html).toBe('<button class="cta dukafi-btn">Click</button>')
    })
...
describe('publishPage', () => {
  ...
  // ─── External CSS mode (per-site bundle served at /_dukafi/css/) ──────────────
  ...
  it('emits four <link> tags pointing at the site bundle in external mode', () => {
    ...
    expect(html).toContain('<link rel="stylesheet" href="/_dukafi/css/reset-aaaaaaaaaaaa.css">')
    expect(html).toContain('<link rel="stylesheet" href="/_dukafi/css/framework-bbbbbbbbbbbb.css">')
    expect(html).toContain('<link rel="stylesheet" href="/_dukafi/css/style-cccccccccccc.css">')
    expect(html).toContain('<link rel="stylesheet" href="/_dukafi/css/userStyles-dddddddddddd.css">')
  ...
  it('inline mode (default) still emits a <style> block and no link tags', () => {
    ...
    const { html } = publishPage(page, site, registry)
    expect(html).toContain('<style>')
    expect(html).not.toMatch(/<link\s+rel="stylesheet"\s+href="\/_dukafi\/css\//)
  })
