...
describe('framework generation facade', () => {
  ...
  it('emits one base :root for color, typography, and spacing variables', () => {
    ...
    expect(baseRoot).toContain('--container-width: var(--container-wide)')
    expect(baseRoot).toContain('--card-padding: 1.5rem')
    expect(baseRoot).toContain('--space-vertical: 5rem')
    expect(baseRoot).toContain('--space-horizontal: 1.5rem')
