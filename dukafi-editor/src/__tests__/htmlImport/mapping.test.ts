...
describe('HTML attribute preservation — props.htmlAttributes for ordinary base modules', () => {
  ...
  it('skips class, module-owned attributes, and reserved editor attributes', () => {
    const result = imported(`
      <div class="kept-as-class" data-bg-src="assets/images/shape/heroShape1_1.png" data-dukafi-node="reserved">
...
// ---------------------------------------------------------------------------
// base.outlet — <dukafi-outlet> custom element
// ---------------------------------------------------------------------------

describe('base.outlet — <dukafi-outlet>', () => {
  it('maps <dukafi-outlet> to a childless base.outlet node', () => {
    const node = single('<dukafi-outlet></dukafi-outlet>')
    expect(node.moduleId).toBe('base.outlet')
    expect(node.children ?? []).toHaveLength(0)
  })

  it('preserves built-in and custom outlet semantics', () => {
    const builtIn = single('<dukafi-outlet data-tag="div"></dukafi-outlet>')
    expect(builtIn.props).toMatchObject({ tag: 'div' })

    const custom = single(
      '<dukafi-outlet data-custom-tag="section-heading"></dukafi-outlet>',
    )
  ...
  it('survives import alongside sibling chrome, producing one outlet node', () => {
    const result = importHtml(
      '<header>Top</header><dukafi-outlet></dukafi-outlet><footer>Bottom</footer>',
    )
...
// ---------------------------------------------------------------------------
// base.loop — <dukafi-loop> custom element
// ---------------------------------------------------------------------------

describe('base.loop — <dukafi-loop>', () => {
  it('maps <dukafi-loop> to a configured base.loop node with child variants', () => {
    const result = importHtml(`
      <dukafi-loop
        class="cards"
