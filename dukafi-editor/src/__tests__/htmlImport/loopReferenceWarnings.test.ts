 ...
 *
 * `<dukafi-loop data-source-id="posts">` — a table slug where a source id
...
const VALID = '<dukafi-loop data-source-id="data.rows" data-table-id="posts"><p>x</p></dukafi-loop>'

describe('loop reference warnings', () => {
  it('warns when the source id is a table slug rather than a source id', () => {
    const result = importHtml('<dukafi-loop data-source-id="posts"><p>x</p></dukafi-loop>')
  ...
  it('warns when a loop has no source id at all', () => {
    const result = importHtml('<dukafi-loop><p>x</p></dukafi-loop>')

    expect(result.warnings[0]?.kind).toBe('unknown-loop-source')
  })

  it('warns when data.rows is missing the table it needs', () => {
    const result = importHtml('<dukafi-loop data-source-id="data.rows"><p>x</p></dukafi-loop>')
  ...
  it('reports every bad loop in one payload, and nothing for ordinary markup', () => {
    const result = importHtml(`
      ...
      <dukafi-loop data-source-id="nope-one"><p>a</p></dukafi-loop>
      <dukafi-loop data-source-id="nope-two"><p>b</p></dukafi-loop>
    `)
