 ...
 * `data-dukafi-canvas-dragging` and `data-dukafi-canvas-dragging-pointer-id` on
...
const IFRAME_DOCUMENT_SENTINEL = 'data-dukafi-canvas-document'
const IFRAME_SRC_DOC = `<!doctype html><html ${IFRAME_DOCUMENT_SENTINEL}><head></head><body></body></html>`
const IFRAME_DOCUMENT_FLAG = '__dukafiCanvasDocument'
...
interface IframeFrameSurfaceProps {
   ...
   * carrying `data-dukafi-readonly-*` markers; opens that source for editing.
   */
...
type IframeWithCleanup = HTMLIFrameElement & { _dukafiCleanup?: () => void }
  ...
  function IframeFrameSurface(
    ...
    const attachIframeDoc = (iframe: HTMLIFrameElement | null) => {
      ...
      if (previousIframe && previousIframe !== iframe) {
        previousIframe._dukafiCleanup?.()
        previousIframe._dukafiCleanup = undefined
      }
      ...
      delete iframe.dataset.dukafiCanvasDocumentLoaded
      const captureSrcDoc = () => {
        ...
        setIframeDoc(doc)
        iframe.dataset.dukafiCanvasDocumentLoaded = 'true'
      }
      ...
      cleanableIframe._dukafiCleanup = () => {
        iframe.removeEventListener('load', captureSrcDoc)
        delete iframe.dataset.dukafiCanvasDocumentLoaded
        cleanableIframe._dukafiCleanup = undefined
      }
    }
    ...
    useEffect(() => {
      ...
      const handleDblClick = (event: MouseEvent) => {
        ...
        if (!region) return
        const id = region.getAttribute('data-dukafi-readonly-id')
        const kind = region.getAttribute('data-dukafi-readonly-kind')
    ...
    //      `data-dukafi-canvas-dragging` on `<html>` so each iframe knows to
    ...
    useEffect(() => {
      ...
      const isCanvasDragActive = (): { pointerId: number } | null => {
        ...
        if (!html) return null
        if (html.dataset.dukafiCanvasDragging !== '1') return null
        const id = Number(html.dataset.dukafiCanvasDraggingPointerId ?? NaN)
