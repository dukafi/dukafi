 ...
 *
 * Read-only composed nodes carry `data-dukafi-readonly-*` markers; the active
 ...
 * `[data-dukafi-readonly-*]` ancestor therefore mislabels editable content as
 ...
 * read `data-dukafi-readonly-label` / `-kind` / `-id` off the returned element.
 */
export function closestReadonlyRegion(target: EventTarget | null): Element | null {
  if (!isElementLike(target)) return null
  const boundary = target.closest('[data-node-id], [data-dukafi-readonly-label]')
