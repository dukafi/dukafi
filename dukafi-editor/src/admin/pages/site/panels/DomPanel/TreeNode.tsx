    ...
    >
      ...
      */}
      <TreeRow
        ...
        // node id (matches `data-dukafi-node-id` on the canvas) so a single id
        // round-trips between the canvas and the layers tree. `data-dukafi-tag`
        // mirrors the resolved HTML tag so agents can disambiguate two
        // "Container" rows by `[data-dukafi-tag="nav"]` vs `[data-dukafi-tag="footer"]`.
        data-testid={`dom-tree-item-${nodeId}`}
        data-dukafi-node-id={nodeId}
        data-dukafi-tag={htmlTag ?? undefined}
        tabIndex={0}
        onClick={(e) => {
          e.stopPropagation()
