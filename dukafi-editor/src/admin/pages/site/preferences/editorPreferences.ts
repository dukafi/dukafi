...
export const EDITOR_PREFS_KEY = 'dukafi-editor-prefs'
...
/**
 * Preferences written before the Dukafi rename live under the old key. Read
 * them once and migrate, so an existing editor does not silently reset theme,
 * density and hover-preview back to defaults on first load after upgrading.
 * The write path only ever touches EDITOR_PREFS_KEY, so the old key is
 * abandoned as soon as anything is changed.
 */
const LEGACY_EDITOR_PREFS_KEY = 'instatic-editor-prefs'

function readLegacyPrefs(): string | null {
  const legacy = globalThis.localStorage?.getItem(LEGACY_EDITOR_PREFS_KEY) ?? null
  if (legacy === null) return null
  try {
    globalThis.localStorage?.setItem(EDITOR_PREFS_KEY, legacy)
    globalThis.localStorage?.removeItem(LEGACY_EDITOR_PREFS_KEY)
  } catch {
    // Best-effort: if the copy fails we still return the value we read.
  }
  return legacy
}
...
function readEditorPrefs(): EditorPrefs {
  const raw = globalThis.localStorage?.getItem(EDITOR_PREFS_KEY) ?? readLegacyPrefs()
