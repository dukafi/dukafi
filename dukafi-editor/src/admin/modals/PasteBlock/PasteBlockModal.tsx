...
import { parseBlockExport, type DukafiBlockExportFile } from '@core/page-tree'
...
interface PasteBlockModalProps {
  ...
  onInsert: (block: DukafiBlockExportFile) => string | null
}
...
function summarise(block: DukafiBlockExportFile): BlockSummary {
...
export function PasteBlockModal({ open, onClose, onInsert }: PasteBlockModalProps) {
  ...
  const parsed = useMemo(() => {
    ...
    if (!trimmed) return { block: null as DukafiBlockExportFile | null, error: '' }
    let raw: unknown
    try {
    ...
    if (!block) {
      return {
        block: null,
        error: 'Valid JSON, but not a Dukafi block — expected a file with "dukafyExport": "block".',
      }
    }
    return { block, error: '' }
  }, [text])
      ...
      footer={
        <>
          ...
          <Button
            ...
            tooltip={parsed.block ? undefined : 'Paste a Dukafi block first'}
