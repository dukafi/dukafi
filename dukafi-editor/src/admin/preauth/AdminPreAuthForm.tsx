...
export function AdminPreAuthForm({
  ...
  // Matches the server-side fallback in `POST /admin/api/cms/setup`, so a
  // merchant who submits without touching the field gets the same name either
  // way rather than 'My Site' here and 'Dukafi Store' in the database.
  const [siteName, setSiteName] = useState('Dukafi Store')
  ...
  // in place of the default icon AND swap the "Dukafi" label for
  ...
  const brandLabel = publicSite.name ?? 'Dukafi'
