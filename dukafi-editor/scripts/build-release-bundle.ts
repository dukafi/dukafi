...
const bundleName = `dukafi-${version}`
...
async function pinRenderBlueprintImage(path: string): Promise<void> {
  ...
  const image = `ghcr.io/corebunch/dukafi:${version}`
  const updated = contents.replace(/ghcr\.io\/corebunch\/dukafi:[^\s]+/g, image)
  if (updated === contents) {
...
\`\`\`sh
INSTATIC_IMAGE=ghcr.io/corebunch/dukafi:${version} docker compose -f compose.prod.yml -f compose.sqlite.yml up -d
\`\`\`
...
INSTATIC_IMAGE=ghcr.io/corebunch/dukafi:${version} docker compose -f compose.prod.yml up -d
\`\`\`

## Railway image-source install

Use \`ghcr.io/corebunch/dukafi:${version}\` as the Railway service source. Attach a volume at \`/app/storage\` and set:
...
These release-bundle copies are already pinned to \`ghcr.io/corebunch/dukafi:${version}\`.
