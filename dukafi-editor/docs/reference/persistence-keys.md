...
- All client-side persistence keys are prefixed `dukafi-` (or `spotlight:` for Spotlight-specific ones). Don't collide with site / module CSS class names.
...
- The convention: `dukafi-<feature>[-v<version>]`. Bumping `-v<N>` invalidates older shapes silently (the schema's `additionalProperties: true` keeps reads tolerant).
...
|-------------------------------------------|-----------------------------------------------------------------------|-----------------------------------------------------------------|
| `dukafi-editor-prefs`                         | All editor preferences (auto-save, hover-preview, admin theme, UI text size, density, layers options) — see [docs/features/editor-preferences.md](../features/editor-preferences.md) | `src/admin/pages/site/preferences/editorPreferences.ts` → `EDITOR_PREFS_KEY` |
| `dukafi-editor-layout-v2`                     | Per-workspace sidebar widths + open states (site / content / data / media) and floating panel positions | `src/admin/state/workspaceLayoutStorage.ts` → `EDITOR_LAYOUT_STORAGE_KEY` |
| `dukafi-clipboard-v1`                         | The editor clipboard (copy / cut / paste of layer subtrees)            | `src/admin/pages/site/store/clipboard/clipboardStorage.ts` → `CLIPBOARD_STORAGE_KEY` |
| `dukafi-class-usage`                          | Recently-used classes in the ClassPicker autocomplete                 | `src/admin/pages/site/preferences/classUsage.ts` → `CLASS_USAGE_STORAGE_KEY` |
| `dukafi-data-grid-primary-widths-v1`          | Per-table primary-column widths in the Data workspace grid            | `src/admin/pages/data/components/DataGrid/usePrimaryColumnWidth.ts` |
| `dukafi-media-page-view-mode`                 | Shared Media workspace and media-gallery view mode (grid / list)      | `src/admin/pages/media/utils/viewMode.ts`                         |
| `dukafi-media-explorer-view-mode`             | Media Explorer panel view mode (site workspace)                       | `src/admin/pages/site/panels/MediaExplorerPanel/mediaExplorerUtils.ts` → `VIEW_MODE_STORAGE_KEY` |
| `dukafi-module-inserter-v1`                   | Module inserter view mode and recent inserts | `src/admin/pages/site/module-picker/moduleInserterPrefs.ts`      |
| `dukafi-onboarding-dismissed`                 | Dashboard onboarding panel: dismissed / open per-device              | `src/admin/pages/dashboard/hooks/useOnboardingState.ts`         |
...
| Key                                       | Owner                                                                 | Source-of-truth file                                            |
|-------------------------------------------|-----------------------------------------------------------------------|-----------------------------------------------------------------|
| `dukafi-spotlight-pending-action`             | The cross-page-reload action a Spotlight command is waiting for (e.g. step-up then resume) | `src/admin/spotlight/pendingAction.ts`             |

### Cookies (HttpOnly — not directly readable)

| Cookie                                    | Owner                                                                 | Source-of-truth file                                            |
|-------------------------------------------|-----------------------------------------------------------------------|-----------------------------------------------------------------|
| `dukafi_admin_session`                        | Admin session token (raw; hashed before lookup)                       | `server/auth/tokens.ts` → `SESSION_COOKIE_NAME`                 |
...
Stored in the `user_preferences` table — one row per `(user_id, key)`. Keys are namespaced under `dukafi-`. Persisted server-side so they sync across devices.
...
const result = safeParseJson(localStorage.getItem('dukafi-...') ?? '', Schema)
if (!result.ok) throw result.error

// Soft (typical): corruption falls back to defaults
const value = parseJsonWithFallback(
  localStorage.getItem('dukafi-...') ?? '',
  Schema,
  DEFAULTS,
)
```
...
localStorage.setItem('dukafi-...', JSON.stringify(next))
```
...
```text
dukafi-editor-layout-v2    →    dukafi-editor-layout-v3
```
...
const KEY = 'dukafi-my-feature-v1'
...
`localStorage.removeItem('dukafi-...')` resets the user's state. The next read falls back to defaults.

For end-to-end tests, the canonical reset is to clear all `dukafi-` keys:

```ts
for (const key of Object.keys(localStorage)) {
  if (key.startsWith('dukafi-') || key.startsWith('spotlight:')) {
...
| Pattern                                                              | Use instead                                              |
|----------------------------------------------------------------------|----------------------------------------------------------|
| Storing keys without a `dukafi-` prefix                                  | Always prefix `dukafi-` (or `spotlight:` for spotlight-owned) |
| `JSON.parse(localStorage.getItem('dukafi-...') ?? '{}')`                 | `parseJsonWithFallback(raw, Schema, DEFAULTS)`           |
