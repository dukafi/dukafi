...
- **Plugin permissions** govern **what a plugin's code can do via the SDK**. Authored in `dukafi-plugin.config.ts` and emitted into `plugin.json`, then approved at install. See [docs/features/plugin-system.md](../features/plugin-system.md).
