...
{
  id:    'class-abc',
  name:  '__dukafi_scope_<nodeId>',          // generated, never user-facing
  kind:  'class',
  selector: '.__dukafi_scope_<nodeId>',
  order: 0,
...
```css
[data-node-id="node-xyz"].__dukafi_scope_node-xyz {
  border-radius: 12px;
}
```
...
```ts
const name = `__dukafi_scope_${nodeId}`
...
The user never sees `__dukafi_scope_<nodeId>` — the panel shows it as "Custom styles".
...
| Naming a user class `__dukafi_scope_*`                                   | Internal scoped rules use this prefix; keep user-created names free of it (the class-kind validator does not reject the prefix — it's a convention, not a gate) |
