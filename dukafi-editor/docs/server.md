...
const routes: readonly RouteHandler[] = [
  ...
  tryServeLoopRuntimeAsset,        // /_dukafi/loop-runtime.js (fixed CMS asset)
  tryServeLoop,                    // /_dukafi/loop/*       → handlers/cms/loop.ts
  tryServeHoleRuntimeAsset,        // /_dukafi/hole-runtime.js (fixed CMS asset)
  tryServeHole,                    // /_dukafi/hole/*       → handlers/cms/hole.ts
  tryServeModuleJsAsset,           // /_dukafi/module-js/*  → handlers/cms/moduleJs.ts
  tryServePublicForm,              // /_dukafi/form/*       → forms/handler.ts
  tryServeRuntimeAsset,            // /_dukafi/assets/*     → published runtime assets
  tryServeRuntimePackageNamespace, // /_dukafi/runtime/cache/<hash>/<...> → bun install workspace
  tryServeSiteCssNamespace,        // /_dukafi/css/*        → hashed CSS bundles
  tryServeMediaRedirect,           // /_dukafi/media/<adapterId>/<path> → 302 to signed read URL
...
- `/_dukafi/runtime/cache/*` — never falls through to the public-slug renderer
- `/_dukafi/css/*` — never falls through
- `/_dukafi/media/*` — never falls through
...
```text
Cookie: dukafi_admin_session=<token>
    │
    ▼
...
- **Layer A — static-to-disk.** **Every** page is baked at publish time. A fully-static page (no dynamic modules, no request-dependent bindings/loop sources, no VC refs to dynamic VCs) bakes a complete document; a page with dynamic nodes bakes its static **shell** with `<dukafi-hole>` placeholders (the dynamic nodes are Layer C holes). HTML is written to `uploads/published/current/<route>.html`, and the CSS bundles (`/_dukafi/css/…`) and runtime JS (`/_dukafi/assets/…`) are baked into the same slot. The visitor router reads all of these directly off disk (`readArtefact` / `readStaticAsset`) — **a published page never touches the DB for HTML, CSS, or JS.** TTFB ≤ 1.5 ms.
...
```text
            ...
            publishDraftSite / publishDataRow
              ...
              ├── for each page (complete doc, or static shell with <dukafi-hole>):
  ...
  readArtefact            page / row / redirect          /_dukafi/hole/<id>?v=<ver>&u=<url>
...
| `runtime/packageServer.ts`        | Serve per-site `bun install` workspace under `/_dukafi/runtime/cache/`. |

Plus the hole endpoint at `server/handlers/cms/hole.ts` — registered in the router BEFORE `tryServePublicRoute` so `/_dukafi/hole/*` requests never fall through to slug resolution.

Published pages are HTML plus up to four hashed CSS bundles (`reset`, `framework`, `style`, `userStyles`). The ONLY first-party client script for ordinary static pages is the Layer C hole runtime, and it's injected ONLY on pages that contain at least one `<dukafi-hole>`. Fully-static pages ship zero first-party JS from us. Plugins and modules can inject explicit frontend assets through the documented runtime channels.
...
1. **Pick the right layer.**
   ...
   - Top-level (e.g. `/_dukafi/something`) → new `tryServeX` in `server/router.ts`, add to the `routes` array in the right order.
