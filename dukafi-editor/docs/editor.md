...
- **Plugin runtime** (`globalThis.__dukafi`) is installed lazily by `ensurePluginRuntime()` in `pluginRuntimeBootstrap.ts`. It's triggered on first admin-layout mount via `useInstalledEditorPlugins`, so plugin code never runs before login and the runtime download stays off the dashboard critical path.
...
Workspace selections still need bookmarkable query strings without replaying route navigation. `urlState` solves this by operating on `window.history.replaceState` directly — no `dukafi:locationchange` event, no route re-match, just a query-string update that keeps the pathname stable.
...
├── pluginRuntimeBootstrap.ts   ← installs globalThis.__dukafi (lazy)
│
├── layouts/
...
|---|---|---|---|
| `<style id="dukafi-editor-chrome">` | `EditorChromeInjector` | **unlayered** | Editor-only chrome: placeholder, slot-instance, list placeholder, unknown-module fallback |
...
- **Recent:** per-browser local state in `dukafi-module-inserter-v1`, validated with TypeBox before use.
...
- **`src/admin/plugin-host-hooks/`** — React hooks exposed to plugins via `globalThis.__dukafi` (set up by `installPluginRuntime()` in `AuthenticatedAdmin`).
