...
docker pull ghcr.io/corebunch/dukafi:0.0.13
```
...
```sh
docker volume create dukafi-storage
...
docker run -d \
  ...
  -p 3001:3001 \
  -e PORT=3001 \
  -e DATABASE_URL="postgres://user:password@host:5432/dukafi" \
  -e STATIC_DIR=/app/dist \
  -e UPLOADS_DIR=/app/storage/uploads \
  -e INSTATIC_SECRET_KEY="replace-with-output-of-generate-secret-key" \
  -v dukafi-storage:/app/storage \
...
Replace `dukafi:local` with `ghcr.io/corebunch/dukafi:<tag>` when deploying from a published image.
