...
```sh
INSTATIC_IMAGE=ghcr.io/corebunch/dukafi:latest docker compose -f compose.prod.yml -f compose.sqlite.yml up -d
```

Pin a semver tag for predictable upgrades:

```sh
INSTATIC_IMAGE=ghcr.io/corebunch/dukafi:0.0.13 docker compose -f compose.prod.yml -f compose.sqlite.yml up -d
```
...
The maintainer release target is `ghcr.io/corebunch/dukafi`, documented in [release-workflow.md](release-workflow.md).
