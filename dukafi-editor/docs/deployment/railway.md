...
```txt
Image=ghcr.io/corebunch/dukafi:0.0.13
PORT=8080
...
```txt
ghcr.io/corebunch/dukafi:0.0.13
```
...
The `Postgres` prefix is the Railway service name. If the database service is renamed, update the reference to match, for example `${{dukafi-postgres.DATABASE_URL}}`.
...
- Use `ghcr.io/corebunch/dukafi:latest` when you want the service to redeploy whenever the `latest` tag moves.
- Use a semver tag like `ghcr.io/corebunch/dukafi:0.0.13` when you want Railway to stage matching patch or minor updates according to the service's auto-update preference.
