...
End users do not need this page to deploy Instatic. They follow [railway.md](railway.md), [render.md](render.md), [vps.md](vps.md), or [docker-image.md](docker-image.md). Maintainers use this page to keep `ghcr.io/corebunch/dukafi` release tags aligned with source tags and deployment templates.
...
```txt
ghcr.io/corebunch/dukafi:latest
ghcr.io/corebunch/dukafi:<semver>
ghcr.io/corebunch/dukafi:<major>.<minor>
```
...
The checked-in Render Blueprints use `ghcr.io/corebunch/dukafi:latest` for new one-click installs. `scripts/build-release-bundle.ts` rewrites the release-bundle copies to the semver image tag automatically.
...
```txt
corebunch/dukafi-render-sqlite
corebunch/dukafi-render-postgres
```
...
```txt
ghcr.io/corebunch/dukafi:0.0.1
ghcr.io/corebunch/dukafi:0.0
ghcr.io/corebunch/dukafi:latest
```
...
```txt
dukafi-0.0.1-release-bundle.tar.gz
```
...
```sh
docker build -t ghcr.io/corebunch/dukafi:dev .
INSTATIC_IMAGE=ghcr.io/corebunch/dukafi:dev docker compose -f compose.prod.yml up -d
```
...
GHCR (`ghcr.io/corebunch/dukafi`) is the only published registry. It is produced directly by the release workflow, is public, and has no aggressive anonymous pull-rate limits — use it in every Compose file, template, and deployment guide. There is no Docker Hub mirror; if one is ever wanted, add a `Mirror To Docker Hub` job plus `DOCKERHUB_USERNAME` / `DOCKERHUB_TOKEN` repository secrets.

## GHCR Visibility

After the first successful tagged release, open the package page for `ghcr.io/corebunch/dukafi` in GitHub Packages and set visibility to public.

Verify anonymous pulls work:

```sh
docker logout ghcr.io
docker pull ghcr.io/corebunch/dukafi:latest
```
