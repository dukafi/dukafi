...
```sh
docker run --rm \
  -v dukafi-prod_uploads:/uploads:ro \
  -v "$PWD/backups:/backup" \
  alpine \
  tar czf "/backup/dukafi-uploads-$(date +%F).tgz" -C /uploads .
```

If your Compose project name is not `dukafi-prod`, find the actual uploads volume name with `docker volume ls | grep uploads`.
...
cat backups/dukafi-YYYY-MM-DD.sql | docker compose -f compose.prod.yml exec -T postgres \
...
```sh
docker run --rm \
  -v dukafi-prod_uploads:/uploads \
  -v "$PWD/backups:/backup" \
  alpine \
  sh -lc "rm -rf /uploads/* && tar xzf /backup/dukafi-uploads-YYYY-MM-DD.tgz -C /uploads"
```
...
docker compose -f compose.prod.yml -f compose.sqlite.yml cp \
  app:/app/data/snapshot.db "./backups/dukafi-$(date +%F).db"
...
docker compose -f compose.prod.yml -f compose.sqlite.yml cp \
  "./backups/dukafi-YYYY-MM-DD.db" app:/app/data/cms.db
