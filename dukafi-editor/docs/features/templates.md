...
1. Build the chrome on a page with `insertHtml`, including one `<dukafi-outlet>` element where the wrapped content should appear (the importer maps it to a `base.outlet` node).
