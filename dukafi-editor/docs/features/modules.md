...
interface ModuleDefinition<TProps extends Record<string, unknown>> {
   ...
   * Layer A/C auto-wraps it in an `<dukafi-hole>` placeholder and defers
  ...
  /**
   * Optional static fallback rendered into the <dukafi-hole> placeholder at
...
When a page uses such a module, the publisher emits a `<script type="importmap">` mapping `three` to the per-site runtime cache URL (`/_dukafi/runtime/cache/<hash>/three/build/three.module.js`).
