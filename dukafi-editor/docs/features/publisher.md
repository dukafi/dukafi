...
├── renderNode.ts                   — recursive node walker; emits <dukafi-hole> for nodes in dynamicNodeIds
...
- **`RenderAccumulators` (mutable outputs).** `cssMap` (deduped module CSS), `jsMap` (deduped render-emitted module JS), `cspSources` (per-page module CSP requirements), `infiniteLoopIds` (loops that requested the infinite runtime), and `holeNodeIds` (nodes that actually emitted a `<dukafi-hole>`). `publishPage` owns this bag, initialises all five up-front (no lazy `undefined`), and threads the **same instances** by reference down the whole walk; renderers append to them. After the walk, head/body builders read the relevant maps and sets.
...
`findDynamicNodeIds` (`src/core/publisher/dynamicDetection.ts`) classifies every node in a page tree as static or dynamic in a single walk. The result set drives both Layer A's shell-vs-complete decision and Layer C's `<dukafi-hole>` placeholder emission. The rules:
...
The server's `siteCssBundle.ts` and the client's `siteCssBundle.ts` together name each bundle file `<group>-<contentHash>.css`. The publisher emits `<link rel="stylesheet" href="/_dukafi/css/<bundle>-<hash>.css">` per non-empty bundle. `Cache-Control: immutable` (1 year) is safe because the hash changes whenever the content does.
...
- **HTML** — fully-static pages bake to a complete document; pages with dynamic
  nodes bake their static **shell** with `<dukafi-hole>` placeholders (the hole
  runtime hydrates each fragment from `/_dukafi/hole/`). Either way the HTML is on
  disk. A page that fails to render (e.g. a VC ref cycle) is skipped and falls
  through to the live renderer.
- **CSS bundles** — `/_dukafi/css/<bundle>-<hash>.css`, for every page.
- **Runtime JS** — `/_dukafi/assets/<versionId>/…`, for every page.
...
`/_dukafi/hole/` fragment fetch** for a page's dynamic islands.
...
statement right after the slot swap — so a baked `<dukafi-hole data-dukafi-version>`
always matches what the hole endpoint expects (a mismatch would make the
endpoint refuse to hydrate).

The exclusive namespaces `/_dukafi/css/*` (`serveSiteCss`) and `/_dukafi/assets/*`
...
6. `<script type="importmap">` mapping bare specifiers (e.g. `three`) to `/_dukafi/runtime/cache/<hash>/...` URLs
7. Runtime asset `<script>` tags (`scriptTagsForRuntimeAssets`)
8. `<link rel="stylesheet" href="/_dukafi/css/<bundle>-<hash>.css">` per bundle
...
- The module-JS injector (`injectModuleScripts` in `server/publish/moduleJsBundle.ts`) merges `script-src 'self'` through the same `rewriteCspMeta` helper — only when at least one `/_dukafi/module-js/<moduleId>.js` script tag was injected.
...
| `server/publish/runtime/packageServer.ts`       | Serve per-site `bun install` workspace under `/_dukafi/runtime/cache/`. |
| `server/publish/loopRuntime.ts`                 | The loop runtime asset (small JS shim used by certain loop variants).|
| `server/handlers/cms/hole.ts`                   | `GET /_dukafi/hole-runtime.js` (serves `HOLE_RUNTIME_JS`) and `GET /_dukafi/hole/<nodeId>?v=<publishVersion>&u=<page-url>` (renders a node subtree at request time for Layer C islands). |
| `server/handlers/cms/moduleJs.ts`               | `GET /_dukafi/module-js/<moduleId>.js?v=<publishVersion>` — serves a module's render-emitted JS from the memoised site map; validates the untrusted moduleId segment; 404 unknown; `text/javascript`; `cache-control: public, max-age=3600`. |
...
applyPublishedHtmlPipeline(renderedOutput, db)
    ...
    ├─→ Inject per-module published JS: one `<script src="/_dukafi/module-js/<id>.js?v=N" defer data-dukafi-module-js="<id>">` per moduleId in the page's injection set (render-emitted ∪ hole-subtree ∩ site jsMap), sorted; CSP script-src → 'self' iff ≥ 1 tag
...
publishDraftSite (server/publish/publishSite.ts)
    ...
    │
    ├─→ Layer A bake — every page (complete doc, or static shell with <dukafi-hole>):
...
3. **`<dukafi-hole>` fragment responses** at `/_dukafi/hole/<nodeId>?v=<publishVersion>&u=<page-url>` — for dynamic nodes inside otherwise-cacheable pages. Fetched lazily by the IntersectionObserver runtime; shared responses are cached in Layer B, while per-visitor holes bypass it.
