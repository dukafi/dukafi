...
`recentStore.ts` persists the last N executed command ids in localStorage (`dukafi-spotlight-recents`). When the palette opens with an empty query, the recents float to the top.
