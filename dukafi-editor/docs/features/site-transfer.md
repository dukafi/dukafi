...
- Format: `site-bundle-<timestamp>.zip` — `.dukafi/site-bundle.json` plus `media/<storagePath>` entries. The archive manifest schema lives in `src/core/data/bundleArchive.ts`; the internal import payload schema lives in `src/core/data/bundleSchema.ts`.
- Endpoints:
...
├── SiteBundleArchiveManifestSchema — archive `.dukafi/site-bundle.json` shape
├── BUNDLE_ARCHIVE_MANIFEST_PATH    — `.dukafi/site-bundle.json`
...
`.dukafi/site-bundle.json` is the first stored ZIP entry and is a `SiteBundleArchiveManifest`:
...
4. Assembles `.dukafi/site-bundle.json` as the first stored entry.
...
For user-facing ZIP archives, the admin importer builds this preview payload from `.dukafi/site-bundle.json` only; media entries use empty `bytesBase64` placeholders because preview never needs file bytes.
...
Body: the user-facing ZIP archive emitted by export. The manifest must be the first stored entry at `.dukafi/site-bundle.json`; media entries must be stored under `media/<storagePath>`. This is the path used by Super Import when a dropped ZIP is an Instatic transfer archive.
...
2. Reads and validates the first ZIP entry as `.dukafi/site-bundle.json`.
