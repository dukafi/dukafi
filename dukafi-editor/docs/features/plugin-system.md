...
A plugin is a zip package containing a `plugin.json` manifest and one or more bundled JavaScript entrypoints. The SDK CLI usually authors that zip from `dukafi-plugin.config.ts`, then the CMS host loads installed plugins at boot. Each server entrypoint runs in its own Bun `Worker`, and that worker hosts a **QuickJS-WASM sandbox** — no Node, no Bun, no host file system, no environment variables, no network unless explicitly granted. Plugins reach the CMS through SDK surfaces scoped to where they run: `api.plugin.*`, `api.cms.*`, `api.editor.*`, and `api.dashboard.*`.

---

## TL;DR

- **Package shape:** the CLI reads `dukafi-plugin.config.ts` and emits a zip containing `plugin.json` plus entrypoint bundles (`server/index.js`, `editor/index.js`, admin app bundles, `modules/index.js`, `frontend/*.js`, optional `pack/site.json`).
...
- **Permissions:** declared in `dukafi-plugin.config.ts` / `plugin.json`, approved by the site owner at install time, enforced by the SDK at runtime. Outbound network also requires a `networkAllowedHosts` allowlist.
- **CLI:** `bun dukafi-plugin init|lint|build|dev` covers scaffolding, manifest/source validation, bundle build, zip creation, and hot-sync to a running CMS.
...
| `dukafi-plugin` CLI                | `src/core/plugin-sdk/cli/`                |
...
`bun dukafi-plugin build` produces this runtime layout from `dukafi-plugin.config.ts`. Bundle formats are intentionally different per surface:
...
Authors normally write `dukafi-plugin.config.ts` with `definePlugin(...)`; the CLI emits the runtime `plugin.json`. The runtime manifest is still the install-time contract, and `parsePluginManifest` in `src/core/plugins/manifest.ts` is the source of truth.
...
`parsePluginManifest` validates all of these and produces a clear error message. `bun dukafi-plugin lint` runs the same checks before upload. Coherence checks also enforce permission-dependent shape:
...
- The manifest parser rejects an editor entrypoint or app-kind admin page that doesn't declare `editor.code` (`parsePluginManifest` coherence checks; `dukafi-plugin lint` reports the same error pre-upload).
...
1. **`dukafi-plugin build`** emits the correct bundle format for each surface and scans sandboxed bundles for the forbidden literals above.
2. **`dukafi-plugin lint`** runs the same scan plus manifest + permission/allowlist coherence checks. Run this before upload.
...
```jsx
import { StatValue, Widget } from '@dukafi/host-ui'
...
api.cms.loops.registerSource({
  ...
  // fragment lazily via /_dukafi/hole/<nodeId>. SHARED tier: the fragment is
...
When a `base.loop` is bound to a `requestDependent` / `perVisitor` source, the publisher does NOT bake it. It emits a `<dukafi-hole>` placeholder (`display: contents`, so it adds no wrapper box) and injects the ~1.1 KB hole runtime once per page (`/_dukafi/hole-runtime.js?v=<publishVersion>` — versioned so a CMS update busts the cache). The runtime fetches each fragment from `/_dukafi/hole/<nodeId>?v=<version>&u=<page-url>` — lazily via `IntersectionObserver` when the placeholder has visible skeleton content, eagerly on load otherwise — then swaps it in. It forwards the visitor's page path + query, and cookies ride along for `perVisitor` holes. Fully-static pages ship zero JS from the publisher.
...
### Settings — declared in `dukafi-plugin.config.ts` / `plugin.json`
...
`bun dukafi-plugin <command>` runs the SDK CLI at `src/core/plugin-sdk/cli/` (`bun run dukafi-plugin <command>` works too inside this repo). The CLI's primary input is `dukafi-plugin.config.ts`.

```sh
bun dukafi-plugin init acme.hero    # scaffold a module plugin
bun dukafi-plugin init acme.seo --kind content-editor
bun dukafi-plugin lint              # validate config + manifest + sources + bundles
bun dukafi-plugin build             # produce dist/ + .plugin.zip
bun dukafi-plugin dev               # watch + sync into a running CMS
```
...
```ts
import { definePlugin, permissions } from '@dukafi/plugin-sdk'
...
`dukafi-plugin dev` writes built files **directly** into the host's `uploads/plugins/<id>/<version>/`. Subsequent rebuilds are picked up on the next activation cycle.

When running inside the Dukafi monorepo, the CLI auto-detects the host's `uploads/` by walking up the tree. From a separate plugin repo:

```sh
INSTATIC_UPLOADS_DIR=../dukafi/uploads bun dukafi-plugin dev
# or
bun dukafi-plugin dev --uploads ../dukafi/uploads
```

First install still goes through the admin UI (`/admin/plugins` → Upload Plugin) so the owner approves permissions. Every `dukafi-plugin dev` rebuild after that flows in without another upload.
...
1. **Scaffold:**
   ```sh
   bun dukafi-plugin init my-plugin
   cd my-plugin
   ```
2. **Edit `dukafi-plugin.config.ts`.** Pick a namespaced ID (`vendor.product`), declare only the permissions you use, and add resources, settings, admin pages, modules, frontend assets, content access, or packs there.
...
4. **Use SDK builders.** Import from `@dukafi/plugin-sdk` for `definePlugin`, `permissions`, `defineModule`, `definePack`, controls, and types. The CLI emits `plugin.json` and runtime bundles into `dist/`.
5. **Lint:**
   ```sh
   bun dukafi-plugin lint
   ```
6. **Build:**
   ```sh
   bun dukafi-plugin build
   ```
7. **Install via admin UI** (`/admin/plugins` → Upload Plugin), approve permissions.
8. **Iterate** with `bun dukafi-plugin dev`.
...
| Calling a host capability without the matching permission                | Declare it in `dukafi-plugin.config.ts` / `plugin.json` permissions |
...
| Skipping `dukafi-plugin lint` before upload                                  | Always lint — the host scans anyway and refuses the upload   |
...
- Source-of-truth files:
  ...
  - `src/core/plugin-sdk/cli/` — `dukafi-plugin` CLI
