...
|---|---|---|---|
| `dukafi-outlet` | `base.outlet` | none (the CMS content outlet) | **No** |
| `dukafi-loop` | `base.loop` | `sourceId`, `filters.tableId`, `orderBy`, `direction`, `limit`, `offset`, `pagination`, `pageSize`, optional `tag` / `customTag` from `data-*` attrs | Yes |
