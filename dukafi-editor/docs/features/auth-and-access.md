...
- **Sessions** are token-cookie based. Cookie name: `SESSION_COOKIE_NAME` (`dukafi_admin_session`). Tokens are hashed before storage; the cookie carries the raw token.
...
createSession(user, deviceLabel, ip)
    ...
    │
    ▼
Set-Cookie: dukafi_admin_session=<rawToken>; HttpOnly; Secure; SameSite=Lax; Path=/admin
    │
    ▼
...
`POST /admin/api/cms/auth/logout` -> `revokeSessionByHash(db, hash)` -> `Set-Cookie: dukafi_admin_session=; Max-Age=0`.
