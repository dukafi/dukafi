...
- Public submissions go through `POST /_dukafi/form/challenge` and `POST /_dukafi/form/submit`, implemented in `server/forms/handler.ts`.
- The browser runtime ships through the module-JS channel: `base.form`'s render() emits it as `js` when `mode === 'cms'` (`src/modules/base/forms/formRuntimeJs.ts`), published pages load it from `/_dukafi/module-js/base.form.js`, and `server/forms/formRuntime.ts`'s `stampFormPageTokens` stamps `data-dukafi-page-token` + `data-dukafi-page-id` onto every CMS form tag — on baked pages and on hole fragments.
...
1. The runtime requests a short-lived challenge from `/_dukafi/form/challenge` when the form attaches in the browser.
2. The runtime posts values plus the challenge to `/_dukafi/form/submit`.
