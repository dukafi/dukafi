...
- **Storage adapters:** built-in local-disk plus plugin-registered adapters. Non-public-url adapters route through `/_dukafi/media/<adapterId>/<storagePath>` for signed redirects.
...
| `public_path`         | `text`        | `text`          | URL the browser uses: `/uploads/...` for local-disk; `/_dukafi/media/<adapterId>/<storagePath>` for non-public-url adapters |
| `mime_type`           | `text`        | `text`          |                                                                                                 |
...
- **non–`public-url`** — the asset URL is host-owned (`/_dukafi/media/<adapterId>/<storagePath>`); the browser hits the host, which 302-redirects to a freshly-signed read URL.
...
- [docs/architecture.md](../architecture.md) — system overview (request lifecycle includes `/_dukafi/media/`)
