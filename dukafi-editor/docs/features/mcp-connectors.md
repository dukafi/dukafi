...
| `/.well-known/oauth-protected-resource/_dukafi/mcp` | Path-aware protected-resource discovery alias. |
| `/.well-known/oauth-authorization-server` | Authorization-server metadata. |
| `/_dukafi/oauth/register` | Dynamic client registration for public clients. |
| `/admin/ai/oauth/authorize` | Signed-in consent screen. |
| `/_dukafi/oauth/token` | Form-encoded authorization-code and refresh-token exchange. |
...
Hosted MCP client
  ...
  → client calls /_dukafi/mcp with the access token
```
...
```sh
claude mcp add dukafi --transport http http://localhost:3000/_dukafi/mcp \
...
The store MCP catalog also exposes the Style Framework through `get_design_tokens` and `update_design_tokens`. The snapshot covers palette tokens, schemes, installed font mappings, typography and spacing scales, semantic H1–H7/P styles, layout/radius presets, icons, buttons, inputs, and framework preferences. Updates upsert the requested framework records into the draft; a provider should read first, patch only the relevant layer, verify, and publish only when deployment was requested. `get_style_framework_snippet` is the compact usage guide for the canvas: it returns topic-filtered HTML for scheme roles, semantic typography, framework spacing, inline icons, button variants, styled inputs, and connected forms.

`ask_user` provides provider-neutral clarification. A model calls it with one to three text, confirmation, single-choice, or multiple-choice questions when missing context would materially alter an edit. Its `requires_user_input` result explicitly ends the current tool sequence: the client should display the questions and resume from the user's next message. Dukafi's server instructions tell both the Dukafi AI harness and external MCP agents to inspect first, ask only when ambiguity remains, and never mutate or publish after requesting clarification. This tool-level fallback works even when a client does not implement native MCP elicitation UI.
