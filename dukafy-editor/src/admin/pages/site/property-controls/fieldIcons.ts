/**
 * Icon per data-field type, for the binding picker's field rows.
 *
 * Moved here out of the deleted `pages/data` workspace: the CMS data-tables UI
 * is gone, but `DataFieldType` remains the shared field vocabulary that forms,
 * layouts and visual-component parameters all speak (`@core/data/schemas`), so
 * the picker still needs an icon for each.
 */
