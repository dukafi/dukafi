...
describe('selectorPickerModel', () => {
  ...
  it('infers class creation for class-like input and ambient creation for selector-shaped input', () => {
    ...
    expect(classifySelectorCreateInput('md:px-8')).toEqual({ kind: 'class', name: 'md:px-8' })
    expect(classifySelectorCreateInput('hover:bg-blue-600')).toEqual({ kind: 'class', name: 'hover:bg-blue-600' })
    expect(classifySelectorCreateInput('w-[37px]')).toEqual({ kind: 'class', name: 'w-[37px]' })
    expect(classifySelectorCreateInput('text-[color:red]')).toEqual({ kind: 'class', name: 'text-[color:red]' })
    expect(classifySelectorCreateInput('w-1/2')).toEqual({ kind: 'class', name: 'w-1/2' })
  })
})
