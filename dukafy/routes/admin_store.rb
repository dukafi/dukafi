  ...
  def product_form(product, error: nil)
    ...
    <<~HTML
      ...
      <form method="post" action="#{action}" hx-post="#{action}" hx-target="body" hx-push-url="#{product.id ? "/admin/store/products/#{product.id}" : "/admin/store"}">
        ...
        <div class="rich-text" data-rich-text><div class="rich-text-toolbar" aria-label="Description formatting"><button type="button" data-command="bold"><strong>B</strong></button><button type="button" data-command="italic"><em>I</em></button><button type="button" data-command="insertUnorderedList">List</button><button type="button" data-command="formatBlock" data-value="h2">Heading</button><button type="button" data-command="createLink">Link</button></div><div class="rich-text-editor" contenteditable="true" role="textbox" aria-multiline="true">#{product.description_document}</div><input type="hidden" name="description_document" value="#{e(product.description_document)}"></div>
        <div class="actions"><button type="submit">Save product</button><a href="/admin/store">Cancel</a></div>
      </form>
      <script>(()=>{document.querySelectorAll('[data-rich-text]').forEach(root=>{const editor=root.querySelector('[contenteditable]'),input=root.querySelector('input[type=hidden]');const sync=()=>input.value=editor.innerHTML;editor.addEventListener('input',sync);root.closest('form').addEventListener('submit',sync);root.querySelectorAll('[data-command]').forEach(button=>button.addEventListener('click',()=>{let value=button.dataset.value||null;if(button.dataset.command==='createLink')value=prompt('Link URL (https://, /path, or #anchor)');if(value!==null)document.execCommand(button.dataset.command,false,value);editor.focus();sync()}))})})()</script>
    HTML
  end
  ...
  def product_attributes
    {
      ...
      description_document: RichTextSanitizer.call(request.params.fetch("description_document", "")),
    }
  end
