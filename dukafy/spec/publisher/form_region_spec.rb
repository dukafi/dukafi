...
#
# The merchant designs a sign-in box out of ordinary nodes; Dukafy contributes
# the verb on the button, the live area around it, and a `form` frame the
# conditions read. Nothing here emits markup of Dukafy's own.
class FormRegionSpec < Minitest::Test
  ...
  def test_a_baked_region_reveals_itself_so_an_already_signed_in_visitor_is_corrected
    ...
    assert_includes html, "dukafy:account-error from:body"
  end
  ...
  def test_the_re_rendered_region_drops_revealed_so_it_cannot_loop
    ...
    assert_includes html, "dukafy:account-updated from:body"
  end
  ...
  # replace their label with Dukafy's markup — the exact thing the 204 on the
