...
class StyleRulesCssSpec < Minitest::Test
  ...
  def css(rules, breakpoints: [])
    Dukafy::Publisher::StyleRulesCss.call(
      "styleRules" => rules, "breakpoints" => breakpoints
    )
  end

  def test_a_site_with_no_rules_emits_nothing
    assert_equal "", Dukafy::Publisher::StyleRulesCss.call({})
    assert_equal "", css({})
  end
  ...
  # exist in the editor's emitter. Nothing in Dukafy can author either yet, so
