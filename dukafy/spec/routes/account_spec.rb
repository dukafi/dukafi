...
# The storefront account endpoints. Dukafy ships no login form — these are the
...
class AccountSpec < Minitest::Test
  ...
  # and a form whose submit button carries the verb. Nothing in it is a Dukafy
  ...
  def test_registering_signs_the_customer_in
    ...
    assert_includes trigger, "dukafy:account-updated"
  ...
  def test_failures_announce_a_reason_as_an_event
    ...
    assert_equal "Use at least 8 characters.",
                 trigger.fetch("dukafy:account-error").fetch("message")
  end
  ...
  def test_login_rejects_a_wrong_password_without_revealing_the_account_exists
    ...
    wrong = trigger.fetch("dukafy:account-error").fetch("message")
    assert_equal 401, last_response.status

    post "/fragments/account/login", email: "ghost@example.com", password: "correct-horse"
    unknown = trigger.fetch("dukafy:account-error").fetch("message")
