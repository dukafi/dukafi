  ...
  module Publisher
    class HtmlDocument
      def self.call(title:, body:, language: "en", description: nil, css_href: nil, css: nil)
        ...
        %(<!doctype html><html lang="#{CGI.escapeHTML(language)}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>#{CGI.escapeHTML(title)}</title>#{description_tag}#{stylesheet}</head><body>#{body}</body></html>)
      end
    end
  end
end
