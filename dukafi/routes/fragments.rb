...
class Fragments < Roda
  ...
  def account_response(status: 204, reason:, ok: true)
    ...
    event = ok ? "dukafi:account-updated" : "dukafi:account-error"
  ...
  def render_cart_subtree(document, node, prefetched: nil, current_entry: nil)
    ...
    Dukafi::Publisher::RenderPage.call(
      document: subtree, registry: Dukafi::Publisher::REGISTRY,
  ...
  #
  # This keeps the contract merchants already have: Dukafi fires events, the
  # merchant's own markup reacts. Listen with
  # `hx-trigger="dukafi:cart-error from:body"`, or read `event.detail.message`.
  #
  ...
  def cart_error_headers(message)
    {
      ...
      "hx-trigger" => JSON.generate({ "dukafi:cart-error" => { "message" => message } }),
    }
  end
  ...
  # Shared reply for the mutating cart endpoints. Fires `dukafi:cart-updated`
  ...
  def cart_mutation_response(node_id)
    ...
    response["HX-Trigger"] = "dukafi:cart-updated"
  ...
  # A line-level change. Distinct from `dukafi:cart-updated` on purpose: the
  ...
  def cart_line_response(node_id, sku)
    response["Content-Type"] = "text/html; charset=utf-8"
    response["Cache-Control"] = "no-store"
    response["HX-Trigger"] = "dukafi:cart-line-updated"
  ...
  def payment_region_html(node_id, attempt)
    frame = {
      ...
      "amountDisplay" => Dukafi::Publisher::StoreModules.format_price(
  ...
  route do |r|
    ...
    # ── Customer accounts ────────────────────────────────────────────────
    # Dukafi ships no login form. These are the verbs; the merchant designs
    ...
    r.on("cart") do
      r.post("items") do
        ...
        # label and replaced it with Dukafi's markup, which is exactly the
        ...
        # `dukafi:cart-updated` event below, so the visitor still sees the
