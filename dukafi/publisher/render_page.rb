...
class Dukafi
  module Publisher
    class RenderPage
      ...
      # Cart verbs a merchant can attach to ANY node. Dukafi ships no stepper
      ...
      CART_ACTIONS = {
        ...
        # a Dukafi variant-picker module.
        "cart.addItem" => {
        ...
        # Auth verbs. Dukafi ships no login form — these are what a merchant
      ...
      def apply_actions(html, node, definition)
        ...
        attrs += if @cart_loop_id
          ...
          # `dukafi:cart-line-updated` event the endpoint fires bring the
      ...
      def cart_lines_placeholder(node, definition)
        ...
        @css.add(definition.id, loop_output[:css])
        collect_runtimes([:htmx])
        html = %(<div class="dukafy-cart-lines dukafy-cart-lines--loading" data-node="#{node_id}" hx-get="/fragments/cart/lines?node=#{node_id}" hx-trigger="revealed, dukafi:cart-updated from:body" hx-swap="outerHTML" aria-live="polite"></div>)
        classes = class_names(node)
        classes.any? ? inject_classes(html, classes) : html
      end
      ...
      CART_REGION_LOAD_TRIGGER = "revealed, dukafi:cart-updated from:body, dukafi:cart-line-updated from:body".freeze
      CART_REGION_LIVE_TRIGGER = "dukafi:cart-updated from:body, dukafi:cart-line-updated from:body".freeze
      ...
      FORM_REGION_EVENTS =
        "dukafi:account-updated from:body, dukafi:account-error from:body, " \
        "dukafi:form-error from:body, dukafi:form-submitted from:body".freeze
