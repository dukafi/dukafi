...
class AdminApiSpec < Minitest::Test
  ...
  def test_resubmitting_a_blank_secret_leaves_the_stored_one_alone
    ...
    assert_equal "keep-me", Dukafi::Plugins.find("payhero").settings[:api_token]
    assert_equal 7, Dukafi::Plugins.find("payhero").settings[:channel_id]
  end
  ...
  def test_media_upload_list_and_delete
    setup_and_login
    source = Tempfile.new(["dukafi-upload", ".txt"])
