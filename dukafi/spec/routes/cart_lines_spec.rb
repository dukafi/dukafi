...
# The composable cart: Dukafi ships NO cart markup. A merchant builds the cart
...
class CartLinesSpec < Minitest::Test
  ...
  # row must be wrapped in one container. Nothing here is a Dukafi component.
  ...
  # Dukafi ships no stepper or remove button; it ships these verbs.
  # ---------------------------------------------------------------------
  ...
  def test_update_sets_an_exact_quantity_and_rerenders_only_that_line
    ...
    assert_equal 200, last_response.status
    assert_equal "dukafi:cart-line-updated", last_response.headers.fetch("hx-trigger")
  ...
  def test_creating_an_order_from_the_cart_and_an_identity
    ...
    assert_empty last_response.body
    assert_equal "dukafi:order-created", last_response.headers.fetch("hx-trigger")
    order = Order.first
  ...
  def test_cart_region_fragment_stays_subscribed_after_it_swaps_itself
    ...
    assert_includes last_response.body, "dukafi:cart-updated from:body"
