...
class RenderPageSpec < Minitest::Test
  ...
  def test_walks_bottom_up_escapes_props_and_deduplicates_css
    ...
    result = Dukafi::Publisher::RenderPage.call(document: document, registry: registry)
  ...
  def test_rejects_missing_nodes_and_cycles
    ...
    assert_raises(ArgumentError) do
      Dukafi::Publisher::RenderPage.call(document: missing, registry: registry)
    end

    cyclic = { "rootNodeId" => "root", "nodes" => { "root" => node("root", "base.body", children: ["root"]) } }
    assert_raises(ArgumentError) do
      Dukafi::Publisher::RenderPage.call(document: cyclic, registry: registry)
    end
  end
  ...
  def test_resolves_site_class_names_onto_node_roots_and_body
    ...
    result = Dukafi::Publisher::RenderPage.call(document: document, registry: registry, site: site)
  ...
  def test_legacy_relationship_props_still_resolve
    ...
    html = Dukafi::Publisher::RenderPage.call(
      document: document, registry: Dukafi::Publisher::REGISTRY, prefetched: catalogue
    ).html
  ...
  def test_a_wrapper_less_loop_emits_valid_markup_inside_a_select
    ...
    html = Dukafi::Publisher::RenderPage.call(
      document: document, registry: Dukafi::Publisher::REGISTRY, prefetched: {}, current_entry: entry
    ).html
  ...
  def render_token(text, cart: nil, entry: nil)
    Dukafi::Publisher::RenderPage.call(
      document: token_document(text), registry: Dukafi::Publisher::REGISTRY,
  ...
  def render_region(cart: nil)
    Dukafi::Publisher::RenderPage.call(
      document: region_document, registry: Dukafi::Publisher::REGISTRY,
  ...
  def test_a_cart_region_bakes_as_an_empty_self_fetching_shell
    ...
    assert_includes html, 'hx-trigger="revealed, dukafi:cart-updated from:body, dukafi:cart-line-updated from:body"'
  ...
  def test_a_rendered_cart_region_keeps_its_own_refresh_wiring
    ...
    assert_includes html, 'hx-get="/fragments/cart/region?node=r"'
    assert_includes html, "dukafi:cart-updated from:body"
  end
  ...
  def test_the_baked_placeholder_still_loads_itself_on_reveal
    assert_includes render_region.html, 'hx-trigger="revealed, dukafi:cart-updated from:body, dukafi:cart-line-updated from:body"'
  end
  ...
  def render_condition(condition, cart: nil, entry: nil)
    Dukafi::Publisher::RenderPage.call(
      document: conditional_document(condition), registry: Dukafi::Publisher::REGISTRY,
      prefetched: {}, cart: cart, current_entry: entry
    ).html
  end
  ...
  def render_stepper(cart)
    Dukafi::Publisher::RenderPage.call(
      document: stepper_document, registry: Dukafi::Publisher::REGISTRY,
